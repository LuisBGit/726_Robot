...
float mainStartPoint = 0;
float mainEndPoint  = 0;
...
float idealPathX[14] = {4.5, 2 ,0, 8, 8, 2, 1, 7, 7, 1, 2, 6, 6, 2};
float idealPathY[14] = {0,0 , 2, 2, -2, -2, 1, 1, -1, -1, 0.5, 0.5, -0.5, -0.5};
...
void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr& laserScanData)
{
	...
	if (currentState == Roam) {
		...
		} else if (currentLook == strafe) {
			if (checkFront(laserScanData, 0.3)) {
				//ROS_INFO("RIGHT IS BLOCKED");
				currentState = Scan;
				velocityCommand.linear.x = 0;
				velocityCommand.angular.z = 0;
			}
		} else if (currentLook == forward) {
			if (checkFront(laserScanData, 0.3)) {
				//ROS_INFO("RIGHT IS BLOCKED");
				currentState = Scan;
				velocityCommand.linear.x = 0;
		...
		if( laserScanData->ranges[j] < 0.5 )  // If there is an object within 0.5m
			{
				...
				velocityCommand.angular.z = 0.3; // turn left
				break;
			}
		}
	}
