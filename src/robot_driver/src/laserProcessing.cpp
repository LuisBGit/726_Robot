...
void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr& laserScanData) {
 ...
 bool endFound = false;
  ...
  for (int i = 1; i < rangeDataNum - 1; i += 1) {
    ...
    float prev1 = laserScanData->ranges[i -1];
    float next1 = laserScanData->ranges[i +1];
    //float next2 = laserScanData->ranges[i +2];
    float current = laserScanData->ranges[i];


    float prevDiff = current - prev1;
    float nextDiff = next1 - current;
