...
void shapeProcess(const std_msgs::Int8::ConstPtr& msg)
{
...
void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr& laserScanData)
{
	...
	for (int i = 0; i < rangeDataNum; i++) {
		yCoords[i] = laserScanData->ranges[i] * sin(i * laserScanData->angle_increment);
		xCoords[i] = laserScanData->ranges[i] * cos(i * laserScanData->angle_increment);
	}
}
